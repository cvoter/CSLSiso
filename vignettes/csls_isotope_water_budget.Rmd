...
data("psnt_lst", package = "cslsdata")
head(psnt_lst)
```
...
data("psnt_isotopes", package = "cslsdata")
head(psnt_isotopes)
```
...
head(psnt_lake_levels)

data("psnt_gw_levels", package = "cslsdata")
head(psnt_gw_levels)
```
...
data("psnt_stage_vol", package = "cslsdata")
head(psnt_stage_vol)
```
...
data("psnt_dictionary", package = "cslsdata")
head(psnt_dictionary)
```
...
timeseries <- find_overlap_timeseries(weather, psnt_lst, psnt_isotopes, 
                                      psnt_lake_levels, psnt_gw_levels)
...
monthly_weather   <- summarise_weather(weather, psnt_lst, psnt_stage_vol, 
                                       psnt_dictionary, timeseries)
...
monthly_lst       <- summarise_lst(psnt_lst, timeseries)
head(monthly_lst)
```
...
monthly_isotopes  <- summarise_isotopes(psnt_isotopes, psnt_dictionary, timeseries,
                                        static_gw = FALSE, psnt_lake_levels,
                                        psnt_gw_levels, 
...
monthly_dV        <- summarise_dV(psnt_lake_levels, timeseries)
head(monthly_dV)
```
...
This gets us to the point where we can visualize the water balance. 
